...
class TeacherInfo extends StatefulWidget {
  ...
  @override
  _TeacherInfoState createState() => _TeacherInfoState(currentData: currentData, schedule: schedule);
}
...
class _TeacherInfoState extends State<TeacherInfo> {
  ...
  Future<void> _saveAsImage() async {
    try {
      ...
      ui.Image image = await boundary.toImage(pixelRatio: 3.0); // Mejor resolución
        ...
        child: RepaintBoundary(
          ...
          child: CustomPaint(
            size: Size(
              ...
              1000, // Altura personalizada (puede ser enorme para mostrar contenido largo)
            ),
            painter: SchedulePainter(schedule),
          ),
        ),
      ),
    );*/
          ...
          SingleChildScrollView(
            ...
            child: RepaintBoundary(
              ...
              child: CustomPaint(
                ...
                painter: SchedulePainter(schedule: schedule, width: 2400, height: dimentions.height-540),
              ),
            ),
          ),
        ],
...
class SchedulePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final textStyleOutside = TextStyle(color: Colors.black, fontSize: textHeight, fontWeight: FontWeight.w600);
    ...
    textPainter.paint(canvas, Offset(5, textHeight+10 + 0*(height-(textHeight+10))/5+ (height/5-10 - textHeight)/2));
    ...
    textPainter.layout(minWidth: 0, maxWidth: 150);
    textPainter.paint(canvas, Offset(5, textHeight+10 + (height-(textHeight+10))/5+ (height/5-10 - textHeight)/2));
    ...
    textPainter.layout(minWidth: 0, maxWidth: 150);
    textPainter.paint(canvas, Offset(5, textHeight+10 + 2*(height-(textHeight+10))/5+ (height/5-10 - textHeight)/2));
    ...
    textPainter.layout(minWidth: 0, maxWidth: 150);
    textPainter.paint(canvas, Offset(5, textHeight+10 + 3*(height-(textHeight+10))/5+ (height/5-10 - textHeight)/2));
    ...
    textPainter.layout(minWidth: 0, maxWidth: 150);
    textPainter.paint(canvas, Offset(5, textHeight+10 + 4*(height-(textHeight+10))/5+ (height/5-10 - textHeight)/2));
    ...
    for (var item in schedule) {
      ...
      */
      ...
      final endX = 150+(width-150)/9 *((startHours[startTime] ?? 0)+1)-5;
      final y = dayPositions[day] ?? 0.0;

      // Draw rectangle
      canvas.drawRect(
          Rect.fromLTRB(startX, y, endX, y + height/5-10), Paint()
      ..color = colorPalletes[(usedSubjects[item["UnitName"]] ?? 7)].background
      ..style = PaintingStyle.fill);

      // Draw text
      textPainter.text = TextSpan(
        text: '${item["GroupCode"]} - ${item["UnitName"]} (Salón ${item["Classroom"]})\nHorario: ${startTime} - ${endTime}',
        style: TextStyle(color: colorPalletes[(usedSubjects[item["UnitName"]] ?? 7)].font, fontSize: textHeight),
      );
